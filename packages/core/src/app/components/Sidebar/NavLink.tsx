...
function NavLink(props: LinkOptions & { text: string; icon: string; iconOnly?: boolean; shortText?: string }) {
  ...
  return (
    <Link
      ...
      inactiveProps={{
        className: 'bg-secondary bg-opacity-20 text-primary-text hover:opacity-100',
      }}
      title={text}
      {...linkProps}
    >
      ...
      <span
        className={cx(
          'font-medium text-sm',
          iconOnly && 'leading-tight text-center break-words w-full max-w-[52px]'
        )}
      >
{iconOnly ? shortText || (text.length > 9 ? text.slice(0, 9) + '...' : text) : text}
      </span>
    </Link>
  )
}
